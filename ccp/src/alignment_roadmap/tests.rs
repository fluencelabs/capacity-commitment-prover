...
fn alignment_works_if_prover_idle() {
    ...
    let new_epoch =
        EpochParameters::new(test::generate_global_nonce(1), test::generate_difficulty(1));
...
fn applying_same_roadmap_idempotent() {
    ...
    let epoch = EpochParameters::new(test::generate_global_nonce(1), test::generate_difficulty(1));
...
fn new_epoch() {
    ...
    let new_epoch =
        EpochParameters::new(test::generate_global_nonce(2), test::generate_difficulty(1));
...
fn same_epoch_new_jobs() {
    ...
    let epoch = EpochParameters::new(test::generate_global_nonce(2), test::generate_difficulty(1));
    let current_status = CCStatus::Running { epoch };
